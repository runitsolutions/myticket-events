...
function myticket_set_custom_price( $cart_object ){
    ...
    $user_id = isset($_COOKIE['myticket_user_id']) ? sanitize_text_field(wp_unslash($_COOKIE['myticket_user_id'])) : "";
...
function myticket_add_cart_item_custom_data( $cart_item_meta, $product_id ) {
    ...
    if(sizeof($myticket_time)>0){
        ...
        $cart_item_meta['myticket_times'] = (isset($_POST['myticket_times'])) ? sanitize_text_field( $_POST['myticket_times'] ): $myticket_time;
    }
...
function myticket_get_item_data ( $cart_data, $cart_item ) {
    ...
    foreach ( $fields_js as $key => $val ){
        switch($val['fields']['type']['value']){
            ...
            case 'note': 
                if(isset($cart_item['myticket_'.$val['fields']['key']['value']])){
                    echo '<div class="cfield"><b>'.$val['fields']['title']['value'].':</b> <span>'.esc_html__($cart_item['myticket_'.$val['fields']['key']['value']]).'</span></div>'; 
                }else{
                    echo '<div class="cfield"><span>'.$val['fields']['title']['value'].'</span></div>'; 
                }
                break;
            default: 
                $cart_item_default = isset($cart_item[$val['fields']['key']['value']]) ? $cart_item[$val['fields']['key']['value']] : "";
                echo '<div class="cfield"><input '.(!$enabled?'disabled="disabled"':'').' class="cart_pers cart_pers_n" type="text" name="cart['.esc_attr($cart_item['key']).']['.esc_attr($val['fields']['key']['value']).']" data-value="'.esc_attr($cart_item_default).'" value="'.esc_attr($cart_item_default).'" placeholder="'.esc_attr($val['fields']['title']['value'], 'myticket-events').'" '.($val['fields']['required']['value'] == 1 ? 'required':'').' /></div>';
        }
        $warning = true;
    }

    // enable calendar date picker
    if (isset($cart_item['myticket_length'])) if ( '' != $cart_item['myticket_length'] && '1' == get_theme_mod('myticket_calendar', '0') ){
    ...
    if ( $warning ){
        ...
        echo '<div class="update" data-warning="'.esc_attr__( 'Please enter required fields!', 'myticket-events' ).'" data-warning2="'.esc_attr__( 'Please hit update cart button first', 'myticket-events' ).'" data-warning3="'.esc_attr__( 'Please pick up the date first!', 'myticket-events' ).'">'.esc_html__( '*Update cart after changes', 'myticket-events' ).'</div>';
    }
   
    // only display default fileds if custom are not present
    if(sizeof($fields_js)==0){

        print_r("<br><b>".esc_html__( "Date", "myticket-events").":</b> ".esc_html( $cart_item['myticket_date'] ));
        print_r("<br><b>".esc_html__( "Time", "myticket-events").":</b> ".esc_html( $cart_item['myticket_times'] ));
...
function myticket_get_cart_items_from_session( $item, $values, $key ) {
    ...
    if ( array_key_exists( 'myticket_times', $values ) )
    $item[ 'myticket_times' ] = $values['myticket_times'];
    ...
    if(isset($_POST['cart']) && isset($_POST['cart'][$item['key']]) && is_array($_POST['cart'][$item['key']]))
...
/*
 Pass custom cart fields to checkout
 This data can be retrieved under PDF tickets or displayed under order item details in WooCommerce > Orders section.
*/
function myticket_add_order_item_meta($item_id, $cart_item, $order_id) {

    // pass custom checkout fields to final order 
    $allowed = ['myticket_date' => 'date', 'myticket_times' => 'time', 'myticket_venue' => 'venue', 'myticket_address' => 'address', 'myticket_zone' => 'zone', 'myticket_row' => 'row', 'myticket_seats' => 'seat', 'myticket_price' => 'price', 'myticket_cal' => 'calendar', 'myticket_length' => 'days', 'myticket_seat_id' => 'seat_id', 'myticket_cfs' => 'cfs', 'myticket_cf1' => 'cf1', 'myticket_cf2' => 'cf2', 'myticket_cf3' => 'cf3', 'myticket_cf4' => 'cf4', 'myticket_cf5' => 'cf5', 'myticket_cf6' => 'cf6', 'myticket_cf7' => 'cf7', 'myticket_cf8' => 'cf8', 'myticket_cf9' => 'cf9', 'myticket_cf10' => 'cf10'];
    foreach($allowed as $k => $v){ 

        // only pass value if not empty
        if(isset($cart_item->legacy_values[$k]) && strlen($cart_item->legacy_values[$k]) > 0) wc_add_order_item_meta($item_id, $v, $cart_item->legacy_values[$k]);
    }
...
function myticket_split_product_individual_cart_items( $cart_item_data, $product_id ){
    ...
    $user_id = sanitize_text_field(isset($_COOKIE['myticket_user_id']) ? $_COOKIE['myticket_user_id'] : "");
    ...
    if(!empty($user_id)){
        ...
        foreach ($reservations as $key => $value) {
            ...
            if($reservations[$key]['user']==$user_id && $reservations[$key]['type']==1){
                ...
                // add layout fields such as tws, cfs to cart
                foreach($reservations[$key] as $k => $v){

                    // prefix to avoid conflicts with other WP plugins
                    $cart_item_data['myticket_'.$k] = $v;
                    // $cart_item_data[$k] = $reservations[$key][$k];
                    // $reservations[$key][$k]
                }
...
if ( ! function_exists( 'myticket_get_order_data_ajax' ) ) {
    function myticket_get_order_data_ajax() {
        ...
        // Secure file path validation to prevent path traversal (WordPress best practices)
        $status_type = $output['type'];
        // Whitelist allowed types
        $allowed_types = array( 'general', 'individual' );
        if ( ! in_array( $status_type, $allowed_types, true ) ) {
            $status_type = 'general'; // Default to safe value
        }
        
        $template_loaded = false;
        
        // Try theme override first
        $allowed_dir = trailingslashit( get_template_directory() . '/' . MYTICKET_SLUG . '/ticket-status-' . $status_type . '/' );
        $override_template_path = $allowed_dir . 'index.php';
        
        // Normalize paths using WordPress function for cross-platform compatibility
        $template_path_normalized = wp_normalize_path( $override_template_path );
        $allowed_dir_normalized = wp_normalize_path( $allowed_dir );
        
        $allowed_dir_real = realpath( $allowed_dir_normalized );
        
        if ( $allowed_dir_real ) {
            $real_path = realpath( $template_path_normalized );
            if ( $real_path && 0 === stripos( wp_normalize_path( $real_path ), $allowed_dir_real ) && file_exists( $template_path_normalized ) ) {
                include $override_template_path;
                $template_loaded = true;
            }
        }
        
        // Fallback to plugin template if theme override not found
        if ( !$template_loaded ) {
            $plugin_allowed_dir = trailingslashit( MYTICKET_PATH . 'templates/ticket-status-' . $status_type . '/' );
            $plugin_template_path = $plugin_allowed_dir . 'index.php';
            
            // Normalize paths using WordPress function for cross-platform compatibility
            $plugin_template_path_normalized = wp_normalize_path( $plugin_template_path );
            $plugin_allowed_dir_normalized = wp_normalize_path( $plugin_allowed_dir );
            
            $plugin_allowed_dir_real = realpath( $plugin_allowed_dir_normalized );
            
            if ( $plugin_allowed_dir_real ) {
                $plugin_real_path = realpath( $plugin_template_path_normalized );
                if ( $plugin_real_path && 0 === stripos( wp_normalize_path( $plugin_real_path ), $plugin_allowed_dir_real ) && file_exists( $plugin_template_path_normalized ) ) {
...
function myticket_events_init_loco_strings(){
    ...
    $temp = esc_attr__("cf1", 'myticket-events');
    $temp = esc_attr__("cf2", 'myticket-events');
    $temp = esc_attr__("cf3", 'myticket-events');
    $temp = esc_attr__("cf4", 'myticket-events');
    $temp = esc_attr__("cf5", 'myticket-events');
}
