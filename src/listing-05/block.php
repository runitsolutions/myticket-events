...
$eids = explode("\n",$attributes['eventID']); 
$eid = isset($_GET['eid']) ? absint($_GET['eid']) : (isset($eids[0]) ? absint($eids[0]) : 0);
$carturl = '';
$checkouturl = '';
$general_price = '';
$product = '';
...
?><img src="<?php echo MYTICKET_URL . 'assets/block_preview-05.jpg'; ?>" alt="<?php echo esc_attr__('block preview', 'myticket-events'); ?>" />
	<div style="font-size:11px;">
		...
		<?php } else { ?>
			<div><?php echo esc_html__('Important! Please make sure that WooCommerce plugin is installed and activated.', 'myticket-events'); ?></div>
		<?php } ?>
	</div><?php
	...
	<div class="kenzap-hall-layout <?php if (isset($_GET['add-to-cart'])) echo 'dn '; if ($attributes['align']) echo "align" . $attributes['align'] . " ";
		...
		<?php else: 
			...
			if( !empty( $attributes['filterLocations'] ) ) {
				$layout_slug = sanitize_file_name( $attributes['filterLocations'] );
				...
				if ( !empty( $layout_slug ) ) {
					...
					// Verify file is within allowed directory to prevent directory traversal
					...
					if ( $real_path && $allowed_dir_real && 0 === stripos( wp_normalize_path( $real_path ), $allowed_dir_real ) && file_exists( $fl_normalized ) ) {
						$layout = file_get_contents( $fl_normalized );
						if ( $layout !== false ) {
							?>
		...
		if ( !empty( $attributes['tableLayout'] ) ) {
			$table_slug = sanitize_file_name( $attributes['tableLayout'] );
			$table_slug = preg_replace( '/[^a-zA-Z0-9_-]/', '', $table_slug ); // Remove dangerous characters
			
			if ( !empty( $table_slug ) ) {
				$allowed_dir = trailingslashit( get_template_directory() . '/' . MYTICKET_SLUG . '/layouts/' );
				$override_template_path = $allowed_dir . $table_slug . '.php';
				...
				if ( $real_path && $allowed_dir_real && 0 === stripos( wp_normalize_path( $real_path ), $allowed_dir_real ) && file_exists( $template_path_normalized ) ) {
					// Include is safe here because we've validated the path is within allowed directory
