...
jQuery(function ($) {
	...
	$(function() {
		...
		if($(khl).length){
			...
			// search for layout under theme/myticket-events/layouts/ folder
			if(hall.length < 100){
				...
				$.ajax({
					...
					data: {
						'layout': hall,
						'table': 'layout-table',
						'action': 'myticket_events_get_templates'
					},
					beforeSend : function () {
					...
					success: function (data) {
						...
						if ($data.length) {
							...
							if(data.success){
								...
								hall_js = JSON.parse(data.layout);

								hall_js.areas.map(function(area, ai){

									// let area_js = area[ai];
									console.log(area);
									area.seats.points.map(function(point, pi){

										// console.log(points.x);
										// console.log(points.y);
										
										if(point){
											hall_js.areas[ai].seats.points[pi].x = parseInt(point.x * 1000) / 1000;
											hall_js.areas[ai].seats.points[pi].y = parseInt(point.y * 1000) / 1000;
										}
									});
								});

									// let area = hall_js.areas[a];
									// console.log(area);

								//}

								console.log(JSON.stringify(hall_js));
	...
	function toggleZoneReservation(z, hall){
		...
		// reserve	
			...
			var ticket_obj = {zone_id: z, zone_text: zone_text, ticket_id: ticket_id, ticket_text: ticket_text, ticket_row: ticket_row, ticket_price: ticket_price, cfs: hall.cfs, type: "pass" };
			...
			Object.keys(hall.areas[z].seats).map(function(key, index) {
				...
				console.log('key' + key + ' | value' + hall.areas[z].seats.key);
		...
		setTimeout(function(){
			...
			$(".cr, .tx").on("click", function(){
				...
				if(!$("#c"+ticket_id).hasClass("reserved")){
					...
					// structure ticket object 
					var ticket_obj = {zone_id: z, zone_text: zone_text, ticket_id: ticket_id, ticket_text: ticket_text, ticket_row: ticket_row, ticket_price: ticket_price, cfs: hall.cfs, type: "seat" };
